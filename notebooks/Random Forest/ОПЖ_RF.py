# Random Forest для ОПЖ с данными за 2014-2023
import pandas as pd
import numpy as np
...
class OPJForecaster:
    ...
    def calculate_metrics(self, y_true, y_pred):
        ...
        print("\n" + "="*50)
        print("МЕТРИКИ КАЧЕСТВА Random Forest ДЛЯ ОПЖ (2014-2023)")
        print("="*50)
    ...
    def prepare_final_output(self, predictions_df, target_year=2024):
        ...
        final_output = pd.DataFrame({
            ...
            'predictions': predictions_df['ОПЖ_прогноз']
        })
    ...
    def predict_future(self, df, future_years=[2024]):
        ...
        print(f"\nСоздание прогноза ОПЖ на {future_years} год...")
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("МОДЕЛЬ СЛУЧАЙНОГО ЛЕСА ДЛЯ ПРОГНОЗА ОПЖ (2014-2023)")
    print("="*70)
    ...
    opj_forecaster.fit(df_opj)
    
    # Сохранение модели
    opj_forecaster.save_model()
    
    # Прогноз на 2024 год
    future_predictions = opj_forecaster.predict_future(df_opj, [2024])
    
    if future_predictions is not None:
        # Подготовка финального вывода для GitHub пайплайна
        final_output = opj_forecaster.prepare_final_output(future_predictions, 2024)
        ...
        # Вывод статистики
        print(f"\nСТАТИСТИКА ПРОГНОЗА:")
        print(f"Средняя ОПЖ в 2024: {future_predictions['ОПЖ_прогноз'].mean():.2f} лет")
        print(f"Регионов с ростом ОПЖ: {(future_predictions['Изменение_ОПЖ'] > 0).sum()}")
        print(f"Регионов со снижением ОПЖ: {(future_predictions['Изменение_ОПЖ'] < 0).sum()}")
        ...
        full_results_filepath = os.path.join(output_dir, 'full_opj_predictions_results.csv')
